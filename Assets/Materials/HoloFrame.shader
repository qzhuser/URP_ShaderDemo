    ...
    SubShader
    {
		...
		Pass{
			...
			half4 fragReal(v2f i) : SV_Target {
				...
				final+=specular;
				half3 excessive=SAMPLE_TEXTURE2D(_GrowMap,sampler_GrowMap,i.uv.xy*10).rgb*_GrowCol.rgb;
				float a=smoothstep(worldPos.y-_GrowWidth,worldPos.y,_Grow*_heigh);
				final+=lerp(excessive,half3(0,0,0),a);
				if(_Grow==0)
					a=0;
				//float a=_grow>1.0?_grow-1:0.0;
				return half4(final, a);
			}
